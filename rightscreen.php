...
$project_folder = ARCHIVE.DS.'projectFoto';
Directree::$projectDir = $project_folder;
$obj_prj = new Directree();
if (!empty($_POST)) {
    // var_dump($_POST);
    $info = '';
    extract($_POST);
...
if (!empty($_GET['screen'])) {
    ...
    //!d(Directree::$projectDir);
    $obj_prj->getThumb(Directree::$projectDir, $selectedScreen);
    $screenFolderList = $obj_prj->getScreenFolder();
    //!d($screenFolderList);
    $month_list = [];
